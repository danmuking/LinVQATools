custom_imports = dict(
    imports=['generate_net', 'generator_dataset', 'srocc', 'rmse',
             'plcc', 'krcc', 'train_evaluator_hook', 'custom_ema_hook'],
    allow_failed_imports=False)
work_dir = 'work_dir/faster_vqa/swin_patch32_fragment32_imp_ref'
visualizer = dict(
    type='Visualizer',
    vis_backends=[
        dict(
            type='WandbVisBackend',
            init_kwargs=dict(project='faster vqa消融', name='swin patch32 fragment32 imp_ref')
        ),
    ],
)
model = dict(
    ...
    vqa_head=dict(name='VQAHead', in_channels=768*2, fc_in=8 * 7 * 7),
