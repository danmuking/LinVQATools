...
class TestCellRunningMaskAgent(TestCase):
    def test_LModel(self):
        ...
        model = LModel(mask_ratio=0.75)
        inputs = {"video":torch.rand((2, 3, 16, 224, 224))}
        agent.train()
    ...
    def test_lmodel_wrapper(self):
        ...
        inputs = {"video":torch.rand((2,1, 3, 16, 224, 224)),"tem_img":torch.rand((2,1,3,160,160)),"spa_img":torch.rand((2,1,3,224,224))}
        y = model(inputs,gt_label=torch.rand((2)),gt_class=torch.empty(2, dtype=torch.long).random_(5),mode='loss')
        print(y)