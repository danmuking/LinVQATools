...
def sampler(video_path: str, epoch: int):
    ...
    frame_sampler = FragmentSampleFrames(fsize_t=16, fragments_t=2, frame_interval=2, num_clips=1, )
    ...
    for index, frame_num in enumerate(frame_index):
        ...
        for i, hs in enumerate(hgrids):
            for j, ws in enumerate(wgrids):
                ...
                h_so, h_eo = hs + rnd_h[i][j][int(index/16)], hs + rnd_h[i][j][int(index/16)] + fsize_h
                w_so, w_eo = ws + rnd_w[i][j][int(index/16)], ws + rnd_w[i][j][int(index/16)] + fsize_w
