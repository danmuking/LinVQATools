...
def viewport_alignment(img, p_lat, t_lon, viewport_resolution=(600, 540)):
    ...
    img = img.expand(viewport_num, -1, -1, -1)
    viewport = F.grid_sample(img, grid, align_corners=True)
    return viewport


if __name__ == '__main__':
    x = torch.ones((20, 3, 2048, 4096)).to("cuda:0")
    y = torch.ones((20, 600, 540, 2)).to("cuda:0")
    viewport = F.grid_sample(x, y)
